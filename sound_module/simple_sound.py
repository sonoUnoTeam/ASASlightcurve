...
class reproductorRaw (object):
    def __init__ (self,
        ...
        # Changes to bisound
        pygame.mixer.init(self.f_s, -16, channels = 2, buffer=4095, allowedchanges=pygame.AUDIO_ALLOW_FREQUENCY_CHANGE | pygame.AUDIO_ALLOW_CHANNELS_CHANGE)
    ...
    def pitch_bisound (self, value, play, vol_left=1, vol_right=1):
        if self.logscale:
            value = np.log10(100*value+1)/2 #This is to achieve reasoable values
            print(value)
        if self.mapping == 'frequency':
        ...
        if play:
            ...
            pygame.mixer.Channel(1).set_volume(vol_left, vol_right)
            pygame.mixer.Channel(2).set_volume(vol_left, vol_right)
            return self.sound
...
class simpleSound(object):
    ...
    def make_bisound(self, data, vol_left=1, vol_right=1, x=0):
        try:
            if not (x == -1):
                #Aquí se llama al método que genera y envía la nota a fluidsynth
                self.reproductor.pitch_bisound(data, True, vol_left, vol_right)
            else:
                # Creería que no se esta usando, porque estaba mal escrita y
                # no generaba error, se deja por las dudas.
                self.reproductor.pitch(0)
    ...
    def array_bisound(self, data, index, vol_left=1, vol_right=1, init=0):
        #Se genera un objeto Track
        # try:
        #     output_file = wave.open(path,'w')
        ...
        try:
            rep = self.reproductor
            if index==0:
                self.bisound_buffer=b''
                s = self.reproductor.pitch_bisound(data, False, vol_left, vol_right)
                self.bisound_buffer += s.get_raw()
            else:
                s = self.reproductor.pitch_bisound(data, False, vol_left, vol_right)
                self.bisound_buffer += s.get_raw()
        except Exception as e:
    ...
    def save_filebisound(self, path):
        rep = self.reproductor
        with wave.open(path,'wb') as output_file:
                output_file.setframerate(rep.f_s)
                output_file.setnchannels(1)
                output_file.setsampwidth(2)
                output_file.writeframesraw(self.bisound_buffer)
