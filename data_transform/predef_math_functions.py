...
def normalize(data_x, data_y, init=0):
    ...
    try:
        ...
        if equal_values:
            ...
            #new_y.fill(0)
            return data_x, data_y, True, False
        else:
            #Perform the normalization
            new_y = ((data_y-np.nanmin(data_y)) 
                / (np.nanmax(data_y)-np.nanmin(data_y)))
            return data_x, new_y, True, False
    except Exception as Error:
        return data_x, data_y, False, Error
...
def square(x, y):
    ...
    This method return the new data with a true status or the unchanged
    data with false status and the Error string.
    """    
    try:
        new_y = np.square(y)
        return x, new_y, True, False
    except Exception as Error:
        return x, y, False, Error
